...
const ChatPanel = (function() {
    ...
    const initialize = function() {
        ...
        // Typing (keydown) event for the input form
        $("#chat-input").on("keydown", () => {
            // Do not add this line, it blocks the typing event
            // e.preventDefault();
            // Get the typing user
            const typingUser = Authentication.getUser();
            // Post it
            Socket.postTypingUser(typingUser.username);
            
            // Clear the previous timeout
            clearTimeout(typingTimeout);

            // Set a new timeout to emit "stopped typing" event after 3 seconds of inactivity
            typingTimeout = setTimeout(() => {
                Socket.postStoppedTypingUser(typingUser.username);
            }, 3000);
    ...
    const addTypingUser = function(username) {
        const currentUser = Authentication.getUser();
        // both are strings
        ...
        const cleanCurrentUser = currentUser.username.trim();
        // console.log(cleanUsername, cleanCurrentUser);
        if (cleanUsername != cleanCurrentUser) {
            $("#typing-user").text(cleanUsername + " is typing...");
        };
    };

    const removeTypingUser = function(username) {
        const currentUser = Authentication.getUser();
        const cleanUsername = username.replace(/"/g, '').trim();
        const cleanCurrentUser = currentUser.username.trim();
        if (cleanUsername != cleanCurrentUser) {
    ...
    return { initialize, update, addMessage, addTypingUser, removeTypingUser };
})();
